...
═══════════════════════════════════════════════ */
.bg-section-hdr {
  font-size: .68rem; font-weight: 800; color: var(--text3); text-transform: uppercase;
  letter-spacing: .1em; padding: 10px 6px 6px; display: flex; align-items: center;
  justify-content: space-between; border-top: 1px solid rgba(255,255,255,0.06); margin-top: 8px;
}
.bg-new-row { display: flex; gap: 5px; margin-bottom: 8px; }
.bg-new-inp {
  flex: 1; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1);
  border-radius: 6px; color: var(--text); padding: 5px 9px; font-size: .75rem; font-family: var(--font-ui);
}
.bg-new-inp:focus { border-color: var(--purple); box-shadow: 0 0 0 2px rgba(139,92,246,0.2); }
.bg-add-btn {
  background: var(--grad-purple); border: none; border-radius: 6px; color: #fff;
  font-size: .72rem; font-weight: 700; padding: 5px 10px; cursor: pointer; white-space: nowrap;
}
.bg-add-btn:hover { filter: brightness(1.15); }
.gruppe-card {
  background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.07);
  border-radius: 9px; margin-bottom: 6px; overflow: hidden; transition: border-color .18s;
}
.gruppe-card:hover { border-color: rgba(139,92,246,0.3); }
.gruppe-head {
  display: flex; align-items: center; gap: 6px; padding: 7px 10px;
  cursor: pointer; user-select: none;
}
.gruppe-name { flex: 1; font-size: .79rem; font-weight: 700; color: #c4b5fd; }
.gruppe-count { font-size: .62rem; color: var(--text3); background: rgba(139,92,246,0.12); padding: 1px 7px; border-radius: 99px; }
.gruppe-del-btn { background: none; border: none; color: var(--red); cursor: pointer; font-size: .7rem; padding: 1px 4px; opacity: .5; }
.gruppe-del-btn:hover { opacity: 1; }
.gruppe-body { display: none; padding: 0 10px 8px; }
.gruppe-body.open { display: block; }
.gruppe-actions { display: flex; gap: 5px; margin-bottom: 7px; flex-wrap: wrap; }
.gruppe-start-btn {
  flex: 1; padding: 5px; background: var(--grad-green); border: none; border-radius: 5px;
  color: #fff; font-size: .7rem; font-weight: 700; cursor: pointer; text-align: center;
}
.gruppe-start-btn:hover { filter: brightness(1.15); }
.gruppe-stop-btn {
  flex: 1; padding: 5px; background: var(--grad-red); border: none; border-radius: 5px;
  color: #fff; font-size: .7rem; font-weight: 700; cursor: pointer; text-align: center;
}
.gruppe-stop-btn:hover { filter: brightness(1.15); }
.gruppe-bot-list { display: flex; flex-direction: column; gap: 3px; }
.gruppe-bot-row {
  display: flex; align-items: center; gap: 5px; padding: 3px 6px;
  background: rgba(255,255,255,0.02); border-radius: 5px; font-size: .73rem;
}
.gruppe-bot-name { flex: 1; color: var(--text2); }
.gruppe-bot-status { font-size: .6rem; padding: 1px 6px; border-radius: 99px; font-weight: 700; }
.gruppe-bot-status.laufend { background: rgba(52,211,153,0.15); color: var(--green); border: 1px solid rgba(52,211,153,0.3); }
.gruppe-bot-status.gestoppt { background: rgba(100,100,100,0.1); color: var(--text3); border: 1px solid rgba(100,100,100,0.2); }
.gruppe-bot-rm { background: none; border: none; color: var(--text3); cursor: pointer; font-size: .65rem; }
.gruppe-bot-rm:hover { color: var(--red); }
.gruppe-assign-row { display: flex; gap: 5px; margin-top: 5px; }
.gruppe-assign-sel { flex: 1; background: rgba(0,0,0,0.35); border: 1px solid rgba(255,255,255,0.1); border-radius: 5px; color: var(--text); padding: 4px 8px; font-size: .71rem; }
.gruppe-assign-btn { background: rgba(139,92,246,0.2); border: 1px solid rgba(139,92,246,0.35); border-radius: 5px; color: #c4b5fd; font-size: .7rem; font-weight: 700; padding: 4px 9px; cursor: pointer; }
.gruppe-assign-btn:hover { background: rgba(139,92,246,0.35); }
</style>
</head>
<body>
...
<div id="tab-bot" class="tab-pane">
  <div class="bot-wrap">
    ...
    <div class="bot-sidebar">
      ...
      <!-- Gruppen -->
      <div class="bg-section-hdr">
        🗂️ Gruppen &amp; Profile
        <button class="bg-add-btn" onclick="gruppeNew()" style="font-size:.62rem;padding:3px 8px">+ Gruppe</button>
      </div>
      <div class="bg-new-row" id="bgNewRow" style="display:none">
        <input class="bg-new-inp" id="bgNewName" placeholder="Gruppenname..." onkeydown="if(event.key==='Enter')gruppeCreate()">
        <button class="bg-add-btn" onclick="gruppeCreate()">✓</button>
        <button class="bg-add-btn" onclick="document.getElementById('bgNewRow').style.display='none'" style="background:var(--bg3);border:1px solid var(--border);color:var(--text3)">✕</button>
      </div>
...
function renderEvCond(bot, eid, c, ci) {
  ...
  return `<div class="cond-group ${logik==='oder'?'cond-group-oder':''}" id="evcond-${eid}-${ci}">
    ${logikBadge}
    ...
    <div class="trig-body" id="tb-${t.id}">
      ...
      <div class="te-section">
        <div class="te-section-title">🔎 Bedingungen
          ...
          <button onclick="trigAddCond('${t.id}','activity')" title="Reagiert wenn ein Spieler eine BC-Aktivität ausführt (Kiss, Spank, ...)">🎭 Aktivität</button>
        </div>
...
function renderCond(bot, tid, c, ci) {
  ...
  } else if (c.typ === 'activity') {
    const BC_ACTIVITIES = ['Kiss','Spank','Caress','Pinch','Bite','Pat','Slap','Tickle','Lick','Rub','Grope','MasturbateHand','MasturbateMouth','MasturbateFoot','PenetrateLight','PenetrateFast','PenetrateHard','Inject','Shock','Suckle','Cuddle','Wiggle'];
    const opts = BC_ACTIVITIES.map(a=>`<option value="${a}" ${c.activity===a?'selected':''}>${a}</option>`).join('');
    inner = `
      <select class="cf" style="width:150px" onchange="condField('${tid}',${ci},'activity',this.value)">
        <option value="">– Jede Aktivität –</option>
        ${opts}
      </select>
      <span style="font-size:.62rem;color:var(--text3)">Richtung:</span>
      <select class="cf" style="width:120px" onchange="condField('${tid}',${ci},'richtung',this.value)">
        <option value="any"    ${(!c.richtung||c.richtung==='any')?'selected':''}>👥 Egal (Sender)</option>
        <option value="sender" ${c.richtung==='sender'?'selected':''}>👤 Nur Sender</option>
        <option value="target" ${c.richtung==='target'?'selected':''}>🎯 Nur Ziel</option>
      </select>
      <span style="font-size:.58rem;color:var(--text3)">Vars: {sender} {ziel} {aktivitaet}</span>`;
  }
  const icons = {wort:'💬',zone:'🗺️',zone_rect:'📐',item_traegt:'👗',item_traegt_nicht:'🚫',trigger_war:'🔗',player_betritt:'👋',ev_timer:'⏱',ev_interval:'🔁',rang:'🏆',shop_kauf:'🛒',activity:'🎭'};
  ...
  const logikBadge = ci === 0
    ...
    : `<div style="display:flex;align-items:center;gap:0;margin:2px 0;padding-left:6px">
        ...
        <button onclick="condLogik('${tid}',${ci},'und_oder')" class="logik-btn ${logik==='und_oder'?'active':''}" style="border-radius:0 3px 3px 0" title="Egal – mindestens eine genügt">UND/ODER</button>
      </div>`;
  return `<div class="cond-group ${logik==='oder'?'cond-group-oder':''}">
    ${logikBadge}
...
function trigAddCond(tid, typ) {
  ...
  const defs = {
    ...
    activity:       {typ:'activity', activity:'Kiss', richtung:'any'},
  };
...
function _ok(trig,rohText,typKey,C){
  ...
  return groups.some(g=>g.every(c=>checkOne(c)));
}
...
function _execAct(a,C,vars){
  ...
  try{
    ...
    else if(a.typ==='item_entf'){InventoryRemove(C,a.gruppe);CharacterRefresh(C);ChatRoomCharacterUpdate(C);ok=true;}
    else if(a.typ==='item'){_applyItemAction(a,C);ok=true;}
...
function _tpl(s,v){
  ...
  return(s??'')
    ...
    .replace(/{sender}/gi,v.sender??v.name??'')
    .replace(/{ziel}/gi,v.ziel??v.C?.Name??v.name??'')
    .replace(/{aktivitaet}/gi,v.aktivitaet??v.wort??'')
...
function _procActivity(actName,actContent,senderC,targetC){
  // actName: kurzer Name (z.B. "Kiss"), actContent: voller Content-String aus BC
  const actHaystack=(actName+' '+actContent).toLowerCase();
  _trigs.forEach(trig=>{
    // Nur Trigger mit activity-Bedingung
    const actConds=(trig.bedingungen??[]).filter(c=>c.typ==='activity');
    if(!actConds.length)return;
    // Alle activity-Bedingungen müssen erfüllt sein
    const actMatch=actConds.every(c=>{
      const kw=(c.activity||'').toLowerCase().trim();
      if(!kw)return true; // Leer = jede Aktivität
      return actHaystack.includes(kw);
    });
    if(!actMatch)return;
    // Richtung: wer ist der primäre Charakter?
    const richtung=actConds[0].richtung??'any';
    const C=(richtung==='target'&&targetC)?targetC:senderC;
    // Von-Filter (bezieht sich immer auf den Sender)
    const vonOk=(()=>{
      if(trig.von==='bot')return senderC.MemberNumber===Player.MemberNumber;
      if(trig.von==='whitelist')return(trig.vonNummern||[]).map(Number).includes(Number(senderC.MemberNumber));
      return true;
    })();
    if(!vonOk)return;
    // Andere Bedingungen prüfen (zone, rang, vortrigger etc.) — wort-Conds werden ignoriert/passieren
    if(!_ok(trig,actContent,'activity',C))return;
    const displayAct=actName||actContent||'?';
    const vars={
      name:C.Name, wort:displayAct, typ:'\uD83C\uDFAD Aktivität',
      x:C.X??0, y:C.Y??0, zone:'', C,
      aktivitaet:displayAct,
      sender:senderC?.Name??'',
      ziel:targetC?.Name??'',
    };
    _log('\uD83C\uDFAD Aktivität "'+displayAct+'" | '+senderC.Name+(targetC?' → '+targetC.Name:''));
    _run(trig,vars);
  });
}
...
const _msgH=function(data){
  ...
  if(data.Type==='Activity'){
    const senderC=ChatRoomCharacter.find(c=>c.MemberNumber===data.Sender)
                  ??(Player.MemberNumber===data.Sender?Player:null);
    if(senderC){
      const dict=data.Dictionary??[];
      const targetNum=dict.find(d=>d.Tag==='TargetCharacter')?.MemberNumber;
      const targetC=targetNum
        ?(ChatRoomCharacter.find(c=>c.MemberNumber===targetNum)??(Player.MemberNumber===targetNum?Player:null))
        :null;
      // Aktivitätsname: aus Dictionary extrahieren oder Content verwenden
      const actEntry=dict.find(d=>d.Tag==='ActivityGroup')||dict.find(d=>d.Tag==='ActivityAsset');
      const actName=actEntry?.Name??actEntry?.ActivityGroup??data.Content??'';
      _procActivity(actName,data.Content??'',senderC,targetC);
    }
    return;
  }
...
function botDeployById(id) {
  ...
  b.laufend = true; _saveBots(); renderBotList(); renderGruppen();
  if (_selBotId === id) {
...
function botStopById(id) {
  ...
  b.laufend = false; _saveBots(); renderBotList(); renderGruppen();
...
function _loadGruppen() {
  try {
    const s = localStorage.getItem('BCK_BotGruppen_v1');
    if (s) _botGruppen = JSON.parse(s);
  } catch(e) {}
}

function _saveGruppen() {
  try { localStorage.setItem('BCK_BotGruppen_v1', JSON.stringify(_botGruppen)); } catch(e) {}
}

_loadGruppen();

function gruppeNew() {
  const row = document.getElementById('bgNewRow');
  row.style.display = 'flex';
  document.getElementById('bgNewName').focus();
}

function gruppeCreate() {
  const inp = document.getElementById('bgNewName');
  const name = inp.value.trim();
  if (!name) return;
  _botGruppen.push({ id: 'g' + Date.now(), name, botIds: [] });
  _saveGruppen();
  inp.value = '';
  document.getElementById('bgNewRow').style.display = 'none';
  renderGruppen();
}

function gruppeDelete(gid) {
  if (!confirm('Gruppe löschen?')) return;
  _botGruppen = _botGruppen.filter(g => g.id !== gid);
  _saveGruppen(); renderGruppen();
}

function gruppeToggle(gid) {
  const el = document.getElementById('gbody-' + gid);
  if (el) el.classList.toggle('open');
}

function gruppeAddBot(gid) {
  const sel = document.getElementById('gadd-' + gid);
  const bid = sel?.value;
  if (!bid) return;
  const g = _botGruppen.find(x => x.id === gid); if (!g) return;
  if (!g.botIds.includes(bid)) { g.botIds.push(bid); _saveGruppen(); renderGruppen(); }
}

function gruppeRemoveBot(gid, bid) {
  const g = _botGruppen.find(x => x.id === gid); if (!g) return;
  g.botIds = g.botIds.filter(b => b !== bid);
  _saveGruppen(); renderGruppen();
}

function gruppeStartAll(gid) {
  const g = _botGruppen.find(x => x.id === gid); if (!g) return;
  if (!_connected) { showStatus('❌ Nicht mit BC verbunden', 'error'); return; }
  let gestartet = 0;
  g.botIds.forEach(bid => {
    const b = _bots.find(x => x.id === bid);
    if (b && !b.laufend) { botDeployById(bid); gestartet++; }
  });
  showStatus(`▶️ ${gestartet} Bot(s) gestartet`, 'success');
  renderGruppen();
}

function gruppeStopAll(gid) {
  const g = _botGruppen.find(x => x.id === gid); if (!g) return;
  if (!_connected) { showStatus('❌ Nicht mit BC verbunden', 'error'); return; }
  let gestoppt = 0;
  g.botIds.forEach(bid => {
    const b = _bots.find(x => x.id === bid);
    if (b && b.laufend) { botStopById(bid); gestoppt++; }
  });
  showStatus(`⏹ ${gestoppt} Bot(s) gestoppt`, 'success');
  renderGruppen();
}

function renderGruppen() {
  const el = document.getElementById('gruppenList'); if (!el) return;
  if (!_botGruppen.length) {
    el.innerHTML = '<div style="color:var(--text3);font-size:.7rem;text-align:center;padding:8px 4px">Noch keine Gruppen. Klicke + Gruppe.</div>';
    return;
  }
  el.innerHTML = _botGruppen.map(g => {
    const bots = g.botIds.map(bid => _bots.find(b => b.id === bid)).filter(Boolean);
    const running = bots.filter(b => b.laufend).length;
    const wasOpen = document.getElementById('gbody-' + g.id)?.classList.contains('open');

    const botRows = bots.map(b => `
      <div class="gruppe-bot-row">
        <span class="gruppe-bot-name">${escHtml(b.name)}</span>
        <span class="gruppe-bot-status ${b.laufend ? 'laufend' : 'gestoppt'}">${b.laufend ? '▶' : '⏹'}</span>
        <button class="gruppe-bot-rm" onclick="event.stopPropagation();gruppeRemoveBot('${g.id}','${b.id}')" title="Aus Gruppe entfernen">✕</button>
      </div>`).join('');

    // Dropdown: only bots not already in this group
    const availBots = _bots.filter(b => !g.botIds.includes(b.id));
    const assignOpts = availBots.map(b => `<option value="${b.id}">${escHtml(b.name)}</option>`).join('');

    return `<div class="gruppe-card" id="gcard-${g.id}">
      <div class="gruppe-head" onclick="gruppeToggle('${g.id}')">
        <span class="gruppe-name">🗂️ ${escHtml(g.name)}</span>
        <span class="gruppe-count">${running}/${bots.length} ▶</span>
        <button class="gruppe-del-btn" onclick="event.stopPropagation();gruppeDelete('${g.id}')" title="Gruppe löschen">✕</button>
      </div>
      <div class="gruppe-body ${wasOpen ? 'open' : ''}" id="gbody-${g.id}">
        <div class="gruppe-actions">
          <button class="gruppe-start-btn" onclick="gruppeStartAll('${g.id}')">▶️ Alle starten</button>
          <button class="gruppe-stop-btn" onclick="gruppeStopAll('${g.id}')">⏹ Alle stoppen</button>
        </div>
        <div class="gruppe-bot-list">
          ${botRows || '<div style="color:var(--text3);font-size:.68rem;padding:4px 2px">Noch keine Bots. Bot unten hinzufügen.</div>'}
        </div>
        ${availBots.length ? `<div class="gruppe-assign-row">
          <select class="gruppe-assign-sel" id="gadd-${g.id}">
            <option value="">+ Bot hinzufügen</option>
            ${assignOpts}
          </select>
          <button class="gruppe-assign-btn" onclick="gruppeAddBot('${g.id}')">+ Add</button>
        </div>` : '<div style="color:var(--text3);font-size:.65rem;margin-top:5px">Alle Bots bereits in Gruppe.</div>'}
      </div>
    </div>`;
  }).join('');
}

// Gruppen nach Bot-Operationen neu rendern
const _origBotNew = typeof botNew === 'function' ? botNew : null;
function _refreshGruppenAfterBotOp() { renderGruppen(); }
