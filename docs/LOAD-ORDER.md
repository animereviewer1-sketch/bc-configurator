...
| 7 | `money.js` | Geld-Tab | idbGet/idbSet (persistence.js), Globals aus items.js |
| 8 | `rank.js` | Rang-Tab | idbGet/idbSet (persistence.js), Globals aus items.js |
| 9 | `shop.js` | Shop-Tab | idbGet/idbSet (persistence.js), Globals aus items.js |
| 10 | `inventar.js` | Inventar/Keywarden-Tab | idbGet/idbSet (persistence.js), Globals aus items.js |
| 11 | `bot-data.js` | Bot-Trigger/Aktions-Definitionen | idbGet/idbSet (persistence.js), Globals aus items.js |
| 12 | `bot-ui.js` | Bot-Editor-UI | idbGet/idbSet (persistence.js), bcSend/onBridgeMessage (bridge.js), Globals aus items.js/bot-data.js |
| 13 | `bot-engine.js` | Bot-Code-Generator | Globals aus items.js/bot-data.js |
| 14 | `outfit-import.js` | Outfit-Code-Parser | idbGet/idbSet (persistence.js), Globals aus items.js |
| 15 | `bc-autobackup.js` | Automatische Backups | idbGet/idbSet (persistence.js), Globals aus items.js |

## Guard-Verhalten (SPLIT-04)

`items.js` prüft in seiner **ersten Anweisung** (vor jeder anderen Deklaration), ob die von ihm benötigten Vorläufer-Globals existieren — `idbGet` aus `persistence.js` und `bcSend`/`onBridgeMessage` aus `bridge.js` (`typeof window[...] !== 'function'`). `bot-ui.js` prüft dieselben drei plus zusätzlich `showStatus` aus `items.js` (die vollständige Kette persistence.js → bridge.js → items.js). Fehlt eines, passiert Folgendes, statt eines stillen, tief in einer Tab-Funktion auftretenden `ReferenceError`:
...
Für einen einmaligen, selbst-entfernenden Handler (Muster: `debugOsOutfit`/`OUTFIT_DEBUG_RESULT`) `offBridgeMessage(type, handler)` innerhalb des Handlers aufrufen. Es gibt **keine try/catch-Isolation** zwischen Handlern desselben Typs — ein werfender Handler bricht den Dispatch für die nachfolgenden Handler dieses Aufrufs ab (verbatim-Semantik aus dem früheren `switch`).
...
4. `CORE_SCRIPTS` in `tests/helpers/loadScript.js` um `'modul.js'` ergänzen, an der richtigen Position relativ zu `persistence.js`/`bridge.js`/`items.js`.
5. Diese Doku-Datei nachziehen.

## Tests

`tests/helpers/loadScript.js` exportiert `CORE_SCRIPTS` (aktuell `['persistence.js', 'bridge.js', 'items.js']`) und `expandLoadOrder(files)`. `loadScript(files)` ruft `expandLoadOrder` intern auf und fügt fehlende Kern-Vorläufer automatisch vor dem ersten `'items.js'`-Eintrag ein — bestehende Testaufrufe wie `loadScript(['items.js'])` mussten deshalb nicht angepasst werden, obwohl `persistence.js`/`bridge.js` jetzt existieren.
