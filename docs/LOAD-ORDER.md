...
| 19 | `nova/nova.js` | Oberfläche: Seitenleiste, Befehlspalette, Toasts, Ladezustand beim Tab-Wechsel. Umhüllt `switchTab`/`showStatus` (ruft das Original immer zuerst auf) | zur Laufzeit `switchTab`, `showStatus`, `toggleTweaksPanel` aus items.js/index.html, `bcIcon` aus bc-icons.js (alle `typeof`-geguardet) — muss deshalb **nach** allen Feature-Modulen stehen |

`nova/nova-boot.js` steht zusätzlich als plain `<script src>` im `<head>` (vor dem ersten `<style>`): Es setzt `data-ui="nova"` auf `<html>` und schreibt den Stylesheet-Link `nova/nova.css` mit Cache-Buster, bevor der Body gezeichnet wird. Leistungs-Leitplanken der Oberfläche (kein WebGL/GSAP, keine Endlos-Animationen außer dem Ladespinner, kein `backdrop-filter`) prüft `tests/nova-design.test.js`.
