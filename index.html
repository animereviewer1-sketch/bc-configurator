  ...
  <div class="shop-wrap">
    ...
    <div class="shop-cfg">
      ...
      <div style="margin-top:18px">
        ...
        <label style="font-size:.65rem;color:#f87171;margin-top:8px;display:block">🔒 NoStrip-Ankündigung (Chat, zusätzliche Zeile bei /nostrip):
...
function _handleShopCmd(rohText,buyerC){
  ...
  if(args[1]&&args[1].toLowerCase()==='all'){
    ...
    // NoStrip-Ankündigung beim All-Kauf (je ein Eintrag pro Zielspieler → eine gemeinsame Zeile)
    if(flagNostrip){
      const rawNsAll=shopItem.announceNostripMsg||_shopCfg.announceNostripMsg||
        ('🔒 '+targets.map(tc=>tc.Name).join(', ')+' tragen '+shopItem.icon+' '+shopItem.name+' und können es nicht ablegen.');
      const nsTxtAll=_shopTpl(rawNsAll,displayBuyer,null,shopItem,gesamt,newBal,anzahl,gesamt);
      if(flagWhisper){
        targets.forEach(tc=>ServerSend('ChatRoomChat',{Content:nsTxtAll,Type:'Whisper',Target:tc.MemberNumber}));
      } else {
        ServerSend('ChatRoomChat',{Content:nsTxtAll,Type:'Chat'});
      }
    }
  ...
  // NoStrip-Ankündigung: zusätzliche Chat-Zeile wenn /nostrip Flag aktiv
  if(flagNostrip){
    ...
    const nsTxt=_shopTpl(rawNs,displayBuyer,targetC,shopItem,preisEffektiv,newBal);
    if(flagWhisper){
      ServerSend('ChatRoomChat',{Content:nsTxt,Type:'Whisper',Target:targetC.MemberNumber});
    } else {
